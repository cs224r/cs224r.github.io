...
\item \texttt{expert.py} --- expert policy + data collection \textbf{[TODO: \texttt{Expert.act}]}
...
\begin{enumerate}
    \item \texttt{expert.py::Expert.act}
...
%\noindent The provided \texttt{DDPMSchedule} class in \texttt{networks.py} serves as a useful reference when implementing \texttt{FlowMatchingSchedule}. Both follow the same pattern (a training-time method and a sampling method) but differ in their formulas.
...
\begin{itemize}
    %\item \texttt{Expert.act} in \texttt{expert.py}: the expert targeting logic. In easy mode, target \texttt{gap1\_y}. In hard mode, hover at the midpoint of both gaps, then randomly commit to one when within \texttt{commit\_dist}. Apply EMA smoothing. See the docstring for details.
...
\textbf{\color{red}Include your results table here.}
...
%\noindent You should see $\sim$100--200 average steps. The agent crashes quickly.
...
%\subsection*{2.1 \quad Implementation}



%\textbf{Deliverables:}
%% Changed the below to match the format of problem 1
